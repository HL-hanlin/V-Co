...
def init_distributed_mode(args):
    if args.dist_on_itp:
        ...
        if 'SLURM_NTASKS' in os.environ:
            ...
            # Fallback: nodes * gpus per node
            args.world_size = int(os.environ.get('SLURM_NNODES', 1)) * torch.cuda.device_count()
        # Set up dist_url for multi-node
        if 'MASTER_ADDR' not in os.environ:
            # Get the first node's hostname as master
            node_list = os.environ.get('SLURM_NODELIST', 'localhost')
            # Handle node list format like "node[001-004]" or "node001"
            if '[' in node_list:
                # Extract first node from range
                import re
                match = re.match(r'([^\[]+)\[(\d+)', node_list)
                if match:
                    prefix, first_num = match.groups()
                    os.environ['MASTER_ADDR'] = f"{prefix}{first_num}"
                else:
